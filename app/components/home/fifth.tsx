  ...
  return (
    ...
    <section className="w-full bg-black text-white font-sans py-36 flex flex-col items-center overflow-hidden antialiased">
      ...
      <div className="w-full max-w-5xl mx-auto flex flex-col items-center text-center px-8 relative">
        ...
        {/* 1. БЛОК С МАСКОТАМИ И УВЕЛИЧЕННЫМ СВЕЧЕНИЕМ */}
        {/* Максимальная ширина поднялась с 550px до 700px для большей масштабности */}
        <div className="relative w-full max-w-[700px] aspect-[2/1] mb-16 flex items-center justify-center">
          
          {/* Фиолетовое свечение стало шире и более размытым (blur-[100px]) */}
        ...
        {/* 2. УВЕЛИЧЕННЫЙ ТЕКСТОВЫЙ БЛОК */}
        {/* Шрифты выросли: md:text-5xl -> md:text-6xl, увеличен max-w заголовка */}
        ...
        {/* Текст описания стал крупнее (md:text-lg) */}
        ...
        {/* 3. УВЕЛИЧЕННЫЕ ИНПУТ И КНОПКИ */}
        {/* Форма расширена с max-w-2xl до max-w-3xl, добавлены увеличенные паддинги */}
        <form 
          onSubmit={handleSubmit}
          className="w-full max-w-3xl flex flex-col sm:flex-row items-stretch justify-center gap-4 z-10"
        >
          {/* Внутренний паддинг плашки увеличен до p-2 */}
          ...
          {/* Правая кнопка Copilot подогнана под новые размеры (py-4, px-8, text-base) */}
          <button 
            type="button"
